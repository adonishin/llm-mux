...
func (a *QwenAuthenticator) Login(ctx context.Context, cfg *config.Config, opts *LoginOptions) (*provider.Auth, error) {
	if cfg == nil {
	...
	tokenData, err := authSvc.PollForToken(context.Background(), deviceFlow.DeviceCode, deviceFlow.CodeVerifier)
	if err != nil {
